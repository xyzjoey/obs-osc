...
class SourceContext:
    def __init__(self, obs_source):
        ...
        if self.obs_source is not None:
            self.name = obs.obs_source_get_name(self.obs_source)
            ...
            self._obs_scene_item = obs.obs_scene_sceneitem_from_source(self._obs_parent_scene, self.obs_source)
    ...
    def __exit__(self, *_):
        if self.obs_source is not None:
            obs.obs_source_release(self.obs_source)
    ...
    def find_filter_by_name(self, filter_name):
        obs_filter = obs.obs_source_get_filter_by_name(self.obs_source, filter_name)
    ...
    def set_enabled(self, enabled):
        ...
        # obs.obs_source_set_enabled(self.obs_source, enabled)
    ...
    def set_muted(self, muted):
        obs.obs_source_set_muted(self.obs_source, muted)

    def media_restart(self):
        obs.obs_source_media_restart(self.obs_source)

    def media_stop(self):
        obs.obs_source_media_stop(self.obs_source)

    def media_prev(self):
        obs.obs_source_media_previous(self.obs_source)

    def media_next(self):
        obs.obs_source_media_next(self.obs_source)

    def set_text(self, text):
        obs_data = obs.obs_source_get_settings(self.obs_source)
        obs.obs_data_set_string(obs_data, "text", text)
        obs.obs_source_update(self.obs_source, obs_data)
        obs.obs_data_release(obs_data)

    def is_scene(self):
        return obs.obs_scene_from_source(self.obs_source) is not None
